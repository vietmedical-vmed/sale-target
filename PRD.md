...
| **Thêm khách hàng** | Nút trên **thanh bộ lọc** (chỉ màn chi tiết, chỉ role có quyền sửa) → mở **popup** chọn Khách hàng + PS phụ trách. **Không có ô Miền**: miền là thuộc tính của PS nên suy từ PS đã chọn (PS nhiều miền thì lấy theo địa bàn đã khai báo; client không suy được thì để trống, server điền bằng `mienForPs()`). Bấm **Xác nhận** mới tạo thẻ KH — và thẻ đó vẫn CHƯA vào DB cho tới khi thêm sản phẩm đầu tiên. Đóng bằng Hủy / dấu X / Esc / bấm ra ngoài. |
